<template>
  <div class="order-card">
    <div class="order-header">
      <div class="order-shop">厂商：{{ item?.ordershop }}</div>
      <div class="order-id">订单号：{{ item?.orderid }}</div>
    </div>
    <div class="order-content">
      <div class="order-name">商品名称：{{ item?.ordername }}</div>
      <div class="order-number">商品个数：{{ item?.ordernumber }}</div>
    </div>
    <div class="order-state">
      <div>
        是否发货：<span class="status-tag success">{{ item?.orderSend }}</span>
      </div>
      <div>
        是否收货：<span class="status-tag warning">{{ item?.orderOk }}</span>
      </div>
    </div>
